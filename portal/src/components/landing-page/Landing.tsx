...
const Landing = ({setInfo,setIsConnected}:LandingProps) => {
    ...
    const connectToMetaMask = async () =>{
        ...
        try{
            ...
            const block_explorer_api = `http://${process.env.REACT_APP_BLOCK_EXPLORER_API}`
            const marketplace_address = `${process.env.REACT_APP_MARKETPLACE_ADDRESS}`
            const nft_address = `${process.env.REACT_APP_NFT_ADDRESS}`

            const marketplace_request = await axios.get(`${block_explorer_api}/smart-contracts/${marketplace_address}`);
            const marketplace_abi = marketplace_request.data.abi;

            const nft_request = await axios.get(`${block_explorer_api}/smart-contracts/${nft_address}`);
            const nft_abi = nft_request.data.abi;
